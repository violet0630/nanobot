"""家居安全 Agent - 负责家居安全检查和安全升级，使用 OpenANP SDK"""
...
DID_DIR = Path.home() / ".nanobot" / "did" / "furniture-manager"

# Agent 发现地址（ad.json）
AGENT_AD_URLS = {
    "did:wba:home.local:security-manager": "http://localhost:8000/agent/ad.json",
    "did:wba:home.local:network-data-storage": "http://localhost:8001/agent/ad.json",
}
...
def _init_did_auth() -> DIDWbaAuthHeader:
    ...
    if not did_doc_path.exists() or not private_key_path.exists():
        did_document, keys = create_did_wba_document(
            hostname="home.local", path_segments=["furniture-manager"]
        )
...
@anp_agent(AgentConfig(
    name="FurnitureSecurity",
    did=AGENT_DID,
    prefix="/agent",
    description="家居安全 Agent，负责家居安全检查和安全升级管理",
))
class FurnitureSecurityAgent:
...
app = FastAPI()
app.include_router(FurnitureSecurityAgent.router())
